...
<template>
  <div class="flex w-full flex-col gap-6">
    <h3 class="font-newsreader italic dark:text-white text-xl">
      {{ $t("navigation.works") }}
    </h3>
    <div class="flex w-full flex-col gap-4">
      ...
      >
        <span class="whitespace-nowrap font-medium dark:text-white/60">
          {{ project.name }}
        </span>
        <div class="mx-2 h-[0.1px] w-full bg-muted" />
        <span class="whitespace-nowrap dark:text-white/60">
          {{ project.release === "soon" ? $t("global.soon") + "..." : project.release }}
        </span>
      </NuxtLink>
    </div>
    <NuxtLinkLocale to="/works">
      <span class="font-newsreader italic dark:text-white/60 cursor-pointer">
        {{ $t("global.see_more") }}
      </span>
    </NuxtLinkLocale>
  </div>
